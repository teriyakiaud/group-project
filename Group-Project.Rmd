...
## Useful Skills You'll Learn Today
  1. Determining the number of clusters needed for a k-means cluster analysis 
  2. Clustering two different data sets
  3. Create a 2D representation of analysis
  4. Justify agreement of analysis using Rand index
...
randIndex(ct.km)
```
Our wine data has nearly 0.9 agreement between the wine varietal type and the cluster solution!

If our agreement had not been as high, we would want to use a different clustering method.


We can also create a 2D representation of the cluster solution in order to visually interpret our results. 
```{r}
...
Our next example uses a seed dataset. This dataset contains 7 measurements for geometric properties of 210 kernels from 3 different varieties of wheat. This dataset is publicly available, though we will download it from our github repository as we have already converted it into a csv file.

First, we will create a standardized dataframe. 
```{r}
library(curl)
...
Just as we did in the first example, we need to figure out how many clusters to use.
```{r}
...
As in the wine dataset, we can see a clear decrease in the within group sum of squares between 1, 2, and 3 clusters and then a smaller decrease after. This suggests that 3 is an appropriate number of clusters to use. The figure also shows that there will most likely not be more than 15 clusters.
...
We can compare our results with the results of the original paper. 
...
randIndex(ct.kmseed)
```
Here, we see that our seed data has a 0.96 agreement!
...
**Used for constructing the module**
  ...
  * [M. Charytanowicz, J. Niewczas, P. Kulczycki, P.A. Kowalski, S. Lukasik, and S. Zak. (2012). 'A Complete Gradient Clustering Algorithm for Features Analysis of X-ray Images', in: Information Technologies in Biomedicine, Ewa Pietka, Jacek Kawa (eds.), Springer-Verlag, Berlin-Heidelberg, 2010, pp. 15-24.](https://www.researchgate.net/publication/234073581_Seeds_dataset) 
       (http://www.ibspan.waw.pl/~pakowal/publikacje/Charytanowicz_et_al%20A%20Complete%20Gradient%20Clustering%20Algorithm%20for%20Features%20Analysis%20of%20X-ray%20Images.pdf)  
