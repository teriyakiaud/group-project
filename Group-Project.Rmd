...
A graph similar to a Cattell Scree test, which is used in Principle Components Analysis and Kabacoff, suggests using the NbClust package as a guide for determing the number of clusters. Kabacoff also suggests plotting total within-groups sum of squares against number of clusters in the k-means solution. More information about Scree tests and an example is in *R in Action* section 14.2.1. 
